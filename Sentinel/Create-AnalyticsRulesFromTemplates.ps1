...
Function Get-AzSentinelAnalyticsRuleTemplates ($workspaceName, $resourceGroupName) {    
	...
	$thisProfile = [Microsoft.Azure.Commands.Common.Authentication.Abstractions.AzureRmProfileProvider]::Instance.Profile
	$thisProfileClient = New-Object -TypeName Microsoft.Azure.Commands.ResourceManager.Common.RMProfileClient -ArgumentList ($thisProfile)
	$token = $thisProfileClient.AcquireAccessToken($context.Subscription.TenantId)
	...
	$urlTemplates = "https://management.azure.com/subscriptions/$($subscriptionId)/resourceGroups/$($resourceGroupName)/providers/Microsoft.OperationalInsights/workspaces/$($workspaceName)/providers/Microsoft.SecurityInsights/alertruletemplates?api-version=2022-01-01-preview"
	$urlDataConnectors = "https://management.azure.com/subscriptions/$($subscriptionId)/resourceGroups/$($resourceGroupName)/providers/Microsoft.OperationalInsights/workspaces/$($workspaceName)/providers/Microsoft.SecurityInsights/dataConnectors?api-version=2022-01-01-preview"
	...
	foreach ($template in $resultAlertTemplates) {
		...
		foreach ($requiredDC in $template.properties.requiredDataConnectors) {
			foreach ($existingDC in $resultDataConnectors) {				
				if ($requiredDC.connectorId -eq $existingDC.name) {					
					$dcExist = $true
				}
			}
		}
...
Function New-AzSentinelAnalyticsRulesFromTemplate ($workspaceName, $resourceGroupName, $templates) {    
	...
	foreach ($template in $templates) {
		...
		if ("" -ne $body) {
			...
			$uri = "https://management.azure.com/subscriptions/$($subscriptionId)/resourceGroups/$($resourceGroupName)/providers/Microsoft.OperationalInsights/workspaces/$($workspaceName)/providers/Microsoft.SecurityInsights/alertRules/$($guid)?api-version=2022-01-01-preview"
			try {
